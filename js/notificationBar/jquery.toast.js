(function($){
	var th = null, cf = null, toast = function(m,o){
		...
		if(!th){ 
			...
			if(cf.align == 'left' || cf.align == 'right')
				...
				th.css({left: '50%', margin: '0px 0 0 -' + (cf.width / 2) + 'px'});
		}
		// create toast
		...
		cb.click(function(){
			clearTimeout(to);
			ti.animate({ height: 0, opacity: 0}, 'normal', function(){
				ti.slideUp("normal", function() { $(this).remove();});
	...
	toast.config = { width: 500, align: 'center', /* left, center, right */ closeForStickyOnly: true };
	$.extend({ toast: toast });
})(jQuery);